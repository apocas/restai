...
- **Endpoints**: `POST /v1/chat/completions`, `POST /v1/completions` (legacy), `POST /v1/embeddings` (+ `dimensions`/`encoding_format:base64`/`user`), `GET /v1/models` + `GET /v1/models/{id}`, `POST /v1/moderations`, plus `/v1/images/generations` (`image.py`) and `/v1/audio/transcriptions` (`audio.py`). Non-OpenAI helper: `GET /direct/models`.
