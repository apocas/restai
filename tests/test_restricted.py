...
def test_restricted_cannot_edit_project(client):
...
def test_restricted_cannot_delete_project(client):
    r = client.delete(f"/projects/{project_id}", auth=(restricted_username, restricted_password))
    assert r.status_code == 403


def test_restricted_cannot_clone_project(client):
...
def test_restricted_cannot_ingest_text(client):
...
def test_restricted_cannot_ingest_url(client):
...
def test_restricted_cannot_trigger_sync(client):
    r = client.post(f"/projects/{project_id}/sync/trigger", auth=(restricted_username, restricted_password))
    assert r.status_code == 403


def test_restricted_cannot_create_comment(client):
...
def test_restricted_cannot_edit_own_profile(client):
...
def test_restricted_cannot_unrestrict_self(client):
    r = client.patch(f"/users/{restricted_username}", json={
        "is_restricted": False,
    }, auth=(restricted_username, restricted_password))
    assert r.status_code == 403
...
def test_restricted_cannot_use_direct_embeddings(client):
