...
class TestVerifyWidgetContext:
    ...
    def test_expired_token_rejected(self):
        ...
        with pytest.raises(ValueError, match="expired"):
            verify_context_token(token, SECRET)

    def test_wrong_secret_rejected(self):
        token = _make_token({"sub": "user-1", "exp": int(time.time()) + 3600})
        with pytest.raises(ValueError, match="Invalid"):
            verify_context_token(token, "wrong-secret")

    def test_missing_token_rejected(self):
        with pytest.raises(ValueError, match="Missing"):
            verify_context_token("", SECRET)
...
class TestApplyWidgetContext:
    ...
    def test_template_substitution(self):
        ...
        result = apply_context(prompt, context, prepend_block=False)
    ...
    def test_prepend_block(self):
        ...
        context = {"name": "Alice", "role": "admin"}
        result = apply_context(prompt, context, prepend_block=True)
    ...
    def test_empty_context_is_noop(self):
        prompt = "Hello {{context.name}}."
        result = apply_context(prompt, {}, prepend_block=True)
        assert result == "Hello {{context.name}}."

    def test_none_context_is_noop(self):
        prompt = "Hello."
        result = apply_context(prompt, None, prepend_block=True)
        assert result == "Hello."

    def test_empty_prompt_with_context(self):
        result = apply_context("", {"name": "Alice"}, prepend_block=True)
        assert "name: Alice" in result
