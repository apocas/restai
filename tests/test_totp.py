...
def test_totp_status_initially_disabled():
    with TestClient(app) as client:
        response = client.get(
...
def test_totp_status_after_enable():
    with TestClient(app) as client:
        response = client.get(
            f"/users/{test_username}/totp/status",
            auth=(test_username, test_password),
...
def test_totp_disable_with_password():
    with TestClient(app) as client:
        response = client.post(
            f"/users/{test_username}/totp/disable",
...
def test_totp_setup_overwrites_previous():
    global totp_secret
    with TestClient(app) as client:
        # Setup twice
...
def test_non_admin_cannot_setup_other_user():
    with TestClient(app) as client:
        response = client.post(
...
def test_enforce_only_local_users():
...
def test_cannot_disable_when_enforced():
    with TestClient(app) as client:
