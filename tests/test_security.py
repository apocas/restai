...
def test_invalid_jwt_cookie_returns_401():
    """Use a separate client to avoid polluting shared client cookies."""
    with TestClient(app) as c:
        c.cookies.set("restai_token", "garbage.jwt.token")
        r = c.get("/projects")
        assert r.status_code == 401


def test_expired_jwt_token_returns_401():
    """Craft an expired JWT and verify it's rejected. Uses separate client."""
    with TestClient(app) as c:
        expired_payload = {
            "username": "admin",
        ...
        c.cookies.set("restai_token", expired_token)
        r = c.get("/projects")
        assert r.status_code == 401


def test_malformed_basic_auth_returns_401(client):
...
def test_bearer_with_invalid_apikey_returns_401(client):
    r = client.get(
        "/projects",
...
def test_user_cannot_edit_other_users_project(client):
    r = client.patch(
...
def test_user_cannot_chat_other_users_private_project(client):
    r = client.post(
...
def test_user_cannot_question_other_users_private_project(client):
    r = client.post(
...
def test_user_cannot_create_project_in_other_team(client):
    r = client.post(
        "/projects",
        json={
...
def test_user_cannot_edit_other_team(client):
    r = client.patch(
        f"/teams/{teamB_id}",
...
def test_member_cannot_admin_own_team(client):
...
def test_user_cannot_add_user_to_other_team(client):
    r = client.post(
...
def test_non_admin_cannot_create_team(client):
    r = client.post(
        "/teams",
...
def test_user_cannot_delete_team_they_dont_belong_to(client):
...
def test_user_cannot_edit_other_user(client):
    r = client.patch(
        f"/users/{userB_name}",
...
def test_user_cannot_delete_other_user(client):
...
def test_user_cannot_view_other_users_apikeys(client):
...
def test_user_cannot_self_assign_projects(client):
...
def test_non_admin_cannot_create_llm(client):
    r = client.post(
        "/llms",
        json={
            "name": "sneaky_llm",
...
def test_non_admin_cannot_create_embedding(client):
    r = client.post(
        "/embeddings",
        json={
            "name": "sneaky_emb",
...
def test_cannot_change_project_llm_to_unauthorized(client):
...
def test_any_user_can_list_all_embeddings(client):
...
def test_llm_api_keys_are_masked(client):
...
def test_admin_users_listing_shows_all(client):
...
def test_non_admin_listing_returns_limited_schema(client):
...
def test_cannot_change_project_llm_to_one_not_in_current_team(client):
...
def test_admin_can_change_project_team(client):
...
def test_admin_can_access_any_team(client):
...
def test_create_project_whitespace_name_fails(client):
    r = client.post(
        "/projects",
        json={"name": "   ", "llm": llmA_name, "type": "agent", "team_id": teamA_id},
        auth=USER_A,
...
def test_statistics_non_admin_only_sees_own_projects():
    ...
    with TestClient(app, raise_server_exceptions=False) as c:
        r = c.get("/statistics/top-projects", auth=USER_A)
