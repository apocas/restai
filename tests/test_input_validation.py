...
def test_user_name_validation(client):
    for name in INVALID_NAMES:
        response = client.post(
            "/users",
...
def test_embedding_name_validation(client):
    for name in INVALID_NAMES:
        response = client.post(
            "/embeddings",
            json={
...
def test_llm_invalid_privacy(client):
    response = client.post(
        "/llms",
        json={
            "name": "test-llm",
...
def test_llm_invalid_class_name(client):
    response = client.post(
        "/llms",
        json={
            "name": "test-llm",
...
def test_embedding_invalid_class_name(client):
    response = client.post(
        "/embeddings",
        json={
            "name": "test-emb",
...
def test_project_invalid_type(client):
    response = client.post(
        "/projects",
...
def test_llm_valid_enums_accepted(client):
