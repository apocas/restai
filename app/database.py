...
class DBWrapper:
    ...
    def create_user(self,
                    ...
                    admin: bool = False,
                    private: bool = False) -> UserDatabase:
    ...
    def get_user_by_apikey(self, apikey: str) -> Optional[UserDatabase]:
        user: Optional[UserDatabase] = self.db.query(UserDatabase).filter(UserDatabase.api_key == apikey).first()
        return user

    def get_user_by_username(self, username: str) -> Optional[UserDatabase]:
        user: Optional[UserDatabase] = self.db.query(UserDatabase).filter(UserDatabase.username == username).first()
        return user
    ...
    def get_user_by_id(self, user_id: int) -> Optional[UserDatabase]:
        user: Optional[UserDatabase] = self.db.query(UserDatabase).filter(UserDatabase.id == user_id).first()
        return user
    ...
    def edit_project(self, name: str, projectModel: ProjectModelUpdate) -> bool:
        ...
        if (projectModel.connection is not None and proj_db.connection != projectModel.connection
