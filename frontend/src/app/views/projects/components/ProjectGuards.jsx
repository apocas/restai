...
export default function ProjectGuards({ project }) {
  ...
  if (!summary) {
    return (
      <Card elevation={0} sx={{ ...forensicCardSx, p: 4, textAlign: "center" }}>
  ...
  return (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12}>
        <Card elevation={0} sx={forensicCardSx}>
          <FlexBox justifyContent="space-between">
            <FlexBox>
              <Security sx={{ ml: 2 }} />
              <H4 sx={{ p: 2 }}>{t("projects.edit.knowledge.guardAnalytics.title")}</H4>
            </FlexBox>
            <FlexBox sx={{ mr: 2 }}>
            ...
            <Grid container spacing={2} sx={{ mb: 2 }}>
              <Grid item xs={6} sm={3}>
                <StatCard elevation={1}>
                  ...
                  <Typography variant="caption" color="text.secondary">{t("projects.edit.knowledge.guardAnalytics.totalChecks")}</Typography>
                </StatCard>
              </Grid>
              <Grid item xs={6} sm={3}>
                <StatCard elevation={1}>
                  <Typography variant="h6" color="error.main">{summary.total_blocks}</Typography>
                  <Typography variant="caption" color="text.secondary">{t("projects.edit.knowledge.guardAnalytics.blocked")}</Typography>
                </StatCard>
              </Grid>
              <Grid item xs={6} sm={3}>
                <StatCard elevation={1}>
                  <Typography variant="h6">{(summary.block_rate * 100).toFixed(1)}%</Typography>
                  <Typography variant="caption" color="text.secondary">{t("projects.edit.knowledge.guardAnalytics.blockRate")}</Typography>
                </StatCard>
              </Grid>
              <Grid item xs={6} sm={3}>
                <StatCard elevation={1}>
                  <Typography variant="h6" color="warning.main">{summary.warn_count}</Typography>
                  <Typography variant="caption" color="text.secondary">{t("projects.edit.knowledge.guardAnalytics.warnings")}</Typography>
                </StatCard>
              </Grid>
            </Grid>

            <Typography variant="subtitle2" sx={{ mb: 1 }}>{t("projects.edit.knowledge.guardAnalytics.events")}</Typography>
      ...
      <Grid item xs={12}>
        <Card elevation={0} sx={forensicCardSx}>
          <H4 sx={{ p: 2 }}>{t("projects.edit.knowledge.guardAnalytics.recentBlocked", { count: eventsTotal })}</H4>
          <Divider />
          {events.length === 0 ? (
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  <TableCell sx={{ pl: 2 }}>{t("projects.edit.knowledge.guardAnalytics.date")}</TableCell>
                  ...
                  <TableCell sx={{ pr: 2 }}>{t("projects.edit.knowledge.guardAnalytics.guardResponse")}</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {events.map((e) => (
